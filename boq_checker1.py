...
def check(file1):
    ...
    for info in os.listdir():
        ...
        try:
            ...
            if len(hdd.columns)!=10:
                counter+=1
                superstring = superstring+str(counter)+" .Number of columns is different (Duplicate/Extra Columns found) in HDD MB of "+info+". Count of columns is "+str(len(hdd.columns))+"\n"
                
        except:
            counter+=1
            superstring=superstring+str(counter)+". No HDD MB captured in "+info+""". Standard names are "HDD","R8_HDD","HDD MB","R08_HDD"."""+"\n"
